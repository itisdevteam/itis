...
Value getgenerate(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "It is set with the command line argument -gen (or ITIS.conf setting gen)\n"
...
Value getblocktemplate(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "ITIS is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "ITIS is downloading blocks...");
...
// Release the wallet and main lock while waiting
	...
	result.push_back(Pair("developerfee_wallet", "i4uma19HCrJ9Y3QXTLjaBYkzWghFXAiEHe"));
    return result;
}
