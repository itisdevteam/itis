...
#if defined(NDEBUG)
#error "ITIS cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "ITIS Signed Message:\n";
...
int64_t GetBlockValue(int nHeight)
{
	...
	if (nHeight == 0) return 300003 * COIN;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
	...
	if(nHeight <= 200 && nHeight > 0) { // Masternode rewads
		...
		if (mNodeCoins == 0) {
			...
			LogPrintf("[LIBRA] Adjusting Libra at height %d with %d masternodes (%d % locked ITIS) and %d ITIS supply at %ld\n", nHeight, nMasternodeCount, lockedCoinValue*100, nMoneySupply, GetTime());
